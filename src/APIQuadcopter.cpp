...
/**
 * Returns the channel on which the corresponding quadcopter module shall
 * connect.
 *
 * @return the channel
 */
...
void APIQuadcopter::updateSpeed()
{
	if (this->currentPositionTimestamps[1] - this->currentPositionTimestamps[0] != 0) {
		...
		this->currentSpeedTimestamps[1] =
		    (int) (this->currentPositionTimestamps[0] + this->currentPositionTimestamps[1]) / 2;
	}
}
